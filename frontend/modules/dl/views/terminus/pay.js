var terminusPay = {
    ...
    _remarkClickCallBack: function() {
        ...
        input.blur(function() {
            $.post('?r=dl/terminus/order-remark', {orderId : orderId, content : input.val()}, function(data){
                if(data.code == 200) {
                    input.remove();
                    td.html(input.val());
    ...
    _returnRemarkClickCallBack: function() {
        ...
        var input = $('<textarea rows="3" cols="20">' +content+ '</textarea>');
        input.blur(function() {
            $.post('?r=dl/terminus/return-order-remark', {orderId : orderId, content : input.val()}, function(data){
                if(data.code == 200) {
                    input.remove();
                    td.html(input.val());
