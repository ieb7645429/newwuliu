...
class LogisticsReturnOrder extends \yii\db\ActiveRecord
{
    ...
    public function scenarios()
    {
        ...
        $scenarios[self::SCENARIO_CREATE_2] = ['member_phone','member_name','member_cityid','goods_num','goods_price','receiving_phone','receiving_name','receiving_name_area','shipping_type','make_from_price','freight'];
    ...
    public function setReturnOrderState($data)
    {
    	...
    	{
    		case '大连入库':
    			if($data->order_state == Yii::$app->params['returnOrderStateEmployee'])
    			{
    				$r1 = $this->_setOrderState_1($data, Yii::$app->params['returnOrderStateDriver'], $role);
    			...
    			break;
    		case '大连退货员':
    			if($data->order_state == Yii::$app->params['returnOrderStateDriver'])
    			{
    				$r1 = $this->_setOrderState_1($data, Yii::$app->params['returnOrderStateDelivery'], $role);
...
// 		$data[0]['district'] = $area::findOne($data[0]['receiving_areaid'])->area_name;
     ...
     * 靳健
     */
    public function returnList($logistics_sn,$goods_sn,$where){
        $condition = $this->getOrderCondition($logistics_sn,$goods_sn);
        $orderList = $this::find()->joinWith('memberCity')->joinWith('returnGoods')->joinWith('shippingType')->joinWith('returnOrderTime')->where(['and',$where,$condition])->orderBy('order_id desc')->asArray()->all();
    ...
    public function isTreatment($list,$state){
        foreach($list as $key => $value){
            $arr = ReturnGoods::find()->where(['and','order_id ='.$value['order_id'],'goods_state ='.$state])->asArray()->all();
    ...
    public function buttonType($orderList,$type){
        if($type==1){
            $goods_state_01 = Yii::$app->params['returnOrderStateEmployee'];
        }
        foreach($orderList as $key => $value){
    ...
    public function orderPrint($order_sn,$goods_sn,$where,$source){
        $list = $this->returnList($order_sn,$goods_sn,$where);
     ...
     * 靳健
     */
    public function ajaxReturnOrderEdit($order_arr){
        ...
        foreach($order_arr as $v){
            ...
            if($return_order_model->order_state == 10){//判断待入库状态
                if(!$this->setReturnOrderState($return_order_model)){
                    return false;
                }
            }
        }
    ...
    public function getOrderCondition($logistics_sn,$goods_sn){
        ...
        if(!empty($goods_sn)){
            $condition['logistics_return_order.order_id'] = $this::getOrderId($goods_sn);
        }
        return $condition;
    }
    /**
     ...
     * 靳健
     */
    public function getReturnWhere($order_state,$add_time=null,$time_type = 'add_time',$other=null){
        ...
        $where = ['and','order_state ='.$order_state,$other,$searchTime];
        return $where;
    }
    /**
     * 查询时间添加
