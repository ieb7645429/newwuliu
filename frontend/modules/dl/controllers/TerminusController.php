...
use frontend\modules\dl\models\TerminusUser;
...
use frontend\modules\dl\models\TerminusPay;
...
class TerminusController extends Controller
{
    ...
    public function actionList($driver)
    {
        ...
        $orderList = $LogisticsOrder->getTerminusList(Yii::$app->request->get('LogisticsOrder')['logistics_sn'],Yii::$app->request->get('Goods')['goods_sn'],$type,null,$add_time,$driver_id);
        //分页
        ...
        return $this->render('list',
                [
                        'goods'=>$goods,'order_sn'=>Yii::$app->request->get('LogisticsOrder')['logistics_sn'],'goods_sn'=>Yii::$app->request->get('Goods')['goods_sn'],
    ...
    public function actionMyself(){
        ...
        $type = 2;
        $add_time = $this->getAddTime(Yii::$app->request->get('OrderTime')['ruck_time']);
        $orderList = $LogisticsOrder->getTerminusList(Yii::$app->request->get('LogisticsOrder')['logistics_sn'],\Yii::$app->request->get('Goods')['goods_sn'],$type,null,$add_time,0,Yii::$app->request->get('OrderPrintLog')['terminus']);
        //分页
        $pages = new Pagination(['totalCount' =>count($orderList), 'pageSize' => Yii::$app->params['page_size']]);
        $model = array_slice($orderList,$pages->offset,$pages->limit);
    ...
    public function actionOver(){
        ...
        $type = 6;
        $add_time = $this->getAddTime(Yii::$app->request->get('OrderTime')['ruck_time']);
        $orderList = $LogisticsOrder->getTerminusList(Yii::$app->request->get('LogisticsOrder')['logistics_sn'],\Yii::$app->request->get('Goods')['goods_sn'],$type,null,$add_time);
        //分页
        $pages = new Pagination(['totalCount' =>count($orderList), 'pageSize' => Yii::$app->params['page_size']]);
        $model = array_slice($orderList,$pages->offset,$pages->limit);
    ...
    public function actionGoodsPrint(){
        $LogisticsOrder = new LogisticsOrder();
        if($list = $LogisticsOrder->getTerminusList(Yii::$app->request->post('order_sn'),Yii::$app->request->post('goods_sn'),7,Yii::$app->request->post('order_arr'))){
    ...
    public function actionGoodsPrintOther(){
        $LogisticsOrder = new LogisticsOrder();
        if($list = $LogisticsOrder->getTerminusList(Yii::$app->request->post('order_sn'),Yii::$app->request->post('goods_sn'),5,Yii::$app->request->post('order_arr'))){
           $list = $LogisticsOrder->getGoodsPrice($list,'terminus');
           $orderPrintLog = new OrderPrintLog();
