...
use frontend\modules\dl\models\LogisticsOrderEdit;
...
use frontend\modules\dl\models\BuyOut;
use frontend\modules\dl\models\GoodsInfo;
...
use frontend\modules\dl\models\OrderPriceEditLog;
use backend\modules\dl\models\OrderRemark;
use frontend\modules\dl\models\LogisticsLines;
use frontend\modules\dl\models\DateNum;
        ...
        if ($model->load(Yii::$app->request->post())) {
            try {
                ...
                if(empty($model->employee_id)){
                    ...
                    //订单封车后只允许大开单员修改
                    if($model->order_state>10&&$role!='大连开单员-删除'){
...
//             $goodsSn = $modelGoods->getGoodsSn(Yii::$app->request->post()['LogisticsOrder']['logistics_route_id']);//生成货号
    ...
    private function _getActiveMenu() {
        $arr = array(
            'index' => ['menu' => '/dl/employee/index', 'item' => '/dl/employee/index'],
            'index-over' => ['menu' => '/dl/employee/index', 'item' => '/dl/employee/index-over'],
            'create' => ['menu' => '/dl/employee/index', 'item' => false],
            'view' => ['menu' => '/dl/employee/index', 'item' => false],
            'pay' => ['menu' => '/dl/employee/pay', 'item' => false],
        );
        $role = array_keys(Yii::$app->authManager->getRolesByUser(Yii::$app->user->id))[0];
        if($role==Yii::$app->params['roleEmployeeDelete']){
            $arr['update'] = ['menu' => '/dl/balance-edit/index', 'item' => false];
        }else{
            $arr['update'] = ['menu' => '/dl/employee/index', 'item' => false];
        }
