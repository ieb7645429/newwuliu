...
//            $('#logisticsorder-shipping_type option[value="2"]').remove();
  ...
  _getRouteOption : function(obj){
        var data = {
            ...
            $.ajax({
                 type: "post",
                 url:'?r=hlj/employee/route-option',
                 data:data,
                 async:true,
                 dataType: 'json',
    ...
    _getTerminusOption : function(){
    	var data = {
            ...
            $.ajax({
                 type: "post",
                 url:'?r=hlj/employee/terminus-option',
                 data:data,
                 async:true,
                 dataType: 'json',
                 success:function(data){
...
function userNameExperimental(){
	...
	var pattern = /^\d{7,8}$|^1[34578]\d{9}$/;  // /^1[34578]\d{9}$/; 
...
function judgeMoney()
{
    var nums = $('#logisticsorder-goods_num').val();
    var moneyOfNum = nums * 0.5;

    var freight = $('#logisticsorder-freight').val();

    if(moneyOfNum && freight){
        if(moneyOfNum > freight){
            // var div = $('#logisticsorder-freight').parent().next();
...
function SearchMemberName(Name){
	if($('#logisticsorder-member_name').attr('readonly')=='readonly'){return;}
	    if(Name==''){
        	$("#list").html('');
			 $('#member_name_list').hide('');
        }
		...
		/* if($('#member_phone_list').html().indexOf('li')>0){
		   	 $('#member_phone_list').html('');
	      	 if(event.which==8){
	      	 	 $('#member_phone_list').html('');
	      	 	return;
	      	 }
...
　        // 　 if(!reg.test(Name)){return;}
            $.ajax({
                 type: "post",
                 url:'?r=hlj/employee/member-name-info',
                 data:data,
                 async:true,
                 dataType: 'json',
                 success:function(data){
                	if(data.code==200){
						...
						for(var i=0;i<data.datas.length;i++){
					       ...
					       $('#list').append('<li '+selected+'><ol class="wid-150" id="list_member_name" data-id="'+data.datas[i].id+'">'+data.datas[i].user_truename+'</ol><ol class="wid-150">'+data.datas[i].member_phone+'</ol><ol class="wid-230">'+data.datas[i].member_areainfo+'</ol></li>');
						  //  console.log(data.datas[i].user_truename);
					  }
					//  console.log(data.datas.length);
					  $("#member_name_list").show();
					}
                     else{
                    		$("#list").html('');
			                $('#member_name_list').hide('');
...
**/
var k = 0;
$("#logisticsorder-member_name").on("keydown",function(event){ 
    if(event.which == 40 || event.which == 38 ){
    //  	console.log(k);
   	  //	console.log($("#list li").eq(0).text());
   	    var status = $("#list li").eq(0).attr("class").indexOf("selected"); 
   	 //  console.log(status);
   	   if(k!=0 && status==0){
   	   	k=0;
   	   }
 ...
 //方向键向下
   if(event.which == 40){
	   ...
	   if(k<$("#list li").length-1){
		 	if(k==0){
		 	 $("#list li").removeClass();
             $("#list li").eq(1).addClass('selected'); 
		 	}
		 	else{
		 	 $("#list li").eq(k).removeClass();
			 $("#list li").eq(k+1).addClass('selected');
			 // console.log(k);
		 	}
			  k++;
		  }
  ...
  if(event.which==38){  
	 if(k>-($("#list li").length)){
		 	if(k==0){
		 	 $("#list li").removeClass();
             $("#list li").eq($("#list li").length-1).addClass('selected'); 
		 	}
		 	else{
		 	 $("#list li").eq(k).removeClass();
			 $("#list li").eq(k-1).addClass('selected');
   ...
   if(event.which==13){
     ...
     $('#list li').each(function(index, element){ 
	    var fontSize = $(this).attr("class").indexOf("selected"); 
		if(fontSize != (-1)){
          $('#logisticsorder-member_name').val($(this).find('ol').first().text());
		  $('#logisticsorder-member_name').attr('data-id',$(this).find('ol').first().attr('data-id'));
		  $('#logisticsorder-member_phone').val($(this).find('ol').eq(1).text());
		  	 $('#logisticsorder-member_name').attr('readonly',true);
		 $('#logisticsorder-member_phone').attr('readonly',true);
		 $('#ismodify').val(0);
		 $('#member_name_list').hide();
		 $('#list').html('');
		}
...
$('#modify').click(function(){
  if($('#logisticsorder-member_name').attr('readonly')=='readonly'){
     $('#logisticsorder-member_name').attr('readonly',false);
     $('#logisticsorder-member_phone').attr('readonly',false);
	 $('#ismodify').val( $('#logisticsorder-member_name').attr('data-id'));
  }
