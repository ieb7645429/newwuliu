...
use frontend\modules\hlj\models\SignupForm;
...
//             ['status', 'in', 'range' => [self::STATUS_ACTIVE, self::STATUS_DELETED]],
	...
	public function getMemberInfoAll($condition){
        return self::find()->where($condition)->select('id,user_truename,member_phone,member_areainfo')->asArray()->limit(10)->all();
    }
    /**
     * 获取用户信息，并注册成用户
     * @param unknown $data
     */
    public function userInfo($data, $username, $user_id = null) {
        $modelArea = new Area();
        ...
        $model->member_provinceid= 8;
        $model->member_areaid= '0';
        $model->member_areainfo ='0';
        return $model->signup($user_id);//新用户注册，默认都是是买断
    }
    /**
	*  扫码枪端登陆验证
	*  2017/07/21
...
//         $store = new Store();
        ...
        $count = $userAll::find()->where(['username'=>$username])->count();
...
// 	        return false;
	...
	public function phoneFormat($param){
	    if(preg_match('/^[2-9][0-9]{6,7}$/',$param['username']) || preg_match('/^[1][34578][0-9]{9}$/',$param['username']))
	    {
	        return true;
	    }else{
	        return false;
