...
?>
<?php $form = ActiveForm::begin(['action'=>['driver/index-another'],'method'=>'get'])?>
       ...
       <?php 
        ...
        ?>
          ...
          <td><?= Html::checkbox('print',in_array($value['order_id'],$order_arr)||!isset($_GET['page'])?true:false,['class'=>'order_check checkbox'.$value['order_id'],'value' => $value['order_id']]);?></td>
             ...
             <td><span class="attention"><?=count($value['driverGoods'])?>/<?=$value['goods_num']?></span></td>
             <td>
             ...
             <?php if(!empty($value['driverGoods'])){?>
          <tr>
             <td  class="goodsTableTd" colspan="12">
                        <table class="table">
                            <?php foreach($value['driverGoods'] as $k => $v){?>
