...
//        $('.return_remark').one('click', this._remarkClickCallBack);
    ...
    _confirmCollectionClickCallBack : function() {
       ...
       $.ajax({
           type: "post",
           url:'?r=teller/return-pay-terminus-confirm',
           data:data,
           async:true,
           dataType: 'json',
           success:returnPayTerminusDetails._confirmSuccess
       });
        ...
        input.blur(function() {
            $.post('?r=teller/return-order-remark', {orderId : orderId, content : input.val()}, function(data){
                if(data.code == 200) {
                    input.remove();
                    td.html(input.val());
                    td.one('click', returnPayTerminusDetails._remarkClickCallBack);
                } else {
                    alert(data.msg);
                    input.remove();
                    td.html(content);
    ...
    _remarkEditClickCallBack: function() {
        ...
        returnPayTerminusDetails.order_id = $(this).data('id');
        $.ajax({
            type: "post",
            url:'?r=teller/return-order-remark-init',
            data:{id: returnPayTerminusDetails.order_id},
            async:true,
            success: returnPayTerminusDetails._remarkEditSuccess
        });
    },
    ...
    _remarkEditSuccess : function(data) {
        if(data.code == 200) {
            ...
            button.click(returnPayTerminusDetails._remarkSaveClickCallBack);
    ...
    _remarkSaveClickCallBack : function() {
        $.ajax({
            ...
            data:{id: returnPayTerminusDetails.order_id, content: $('#order_remark').val()},
            async:true,
