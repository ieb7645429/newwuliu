...
//        $('.remark').one('click', this._remarkClickCallBack);
    ...
    _confirmCollectionClickCallBack : function() {
       ...
       $.ajax({
           type: "post",
           url:'?r=teller/income-driver-confirm',
           data:data,
           async:true,
           success:incomeDriverDetails._confirmSuccess
       });
        ...
        input.blur(function() {
            $.post('?r=teller/order-remark', {orderId : orderId, content : input.val()}, function(data){
                if(data.code == 200) {
                    input.remove();
                    td.html(input.val());
                    td.one('click', incomeDriverDetails._remarkClickCallBack);
                } else {
                    alert(data.msg);
                    input.remove();
                    td.html(content);
    ...
    _remarkEditClickCallBack: function() {
        ...
        incomeDriverDetails.order_id = $(this).data('id');
        $.ajax({
            type: "post",
            url:'?r=teller/order-remark-init',
            data:{id: incomeDriverDetails.order_id},
            async:true,
            success: incomeDriverDetails._remarkEditSuccess
        });
    },
    ...
    _remarkEditSuccess : function(data) {
        if(data.code == 200) {
            ...
            button.click(incomeDriverDetails._remarkSaveClickCallBack);
    ...
    _remarkSaveClickCallBack : function() {
        $.ajax({
            ...
            data:{id: incomeDriverDetails.order_id, content: $('#order_remark').val()},
            async:true,
