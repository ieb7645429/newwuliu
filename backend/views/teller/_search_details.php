...
<div class="logistics-order-search">
    ...
    <?= Html::input('hidden', 'member_id', Yii::$app->request->get('member_id'))?>
    <?php endif;?>
    ...
    <?php if(Yii::$app->request->get('driver_member_id')):?>
    <?= Html::input('hidden', 'driver_member_id', Yii::$app->request->get('driver_member_id'))?>
    <?php endif;?>
    ...
    <?= $form->field($model, 'logistics_sn')->textInput(['value' => ArrayHelper::getValue(Yii::$app->request->get('LogisticsOrderSearch'), 'logistics_sn', '')]) ?>
    
    <?= $form->field($model, 'goods_price_state')
             ->label('收款类型')
             ->dropDownList(['0' => '全部', '1'=>'已收', '2'=>'未收'],
                            ['value' => ArrayHelper::getValue(Yii::$app->request->get('LogisticsOrderSearch'), 'goods_price_state', '0')]);
    ?>
    </div>
