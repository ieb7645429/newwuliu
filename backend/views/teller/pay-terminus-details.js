...
//        $('.remark').one('click', this._remarkClickCallBack);
    ...
    _confirmCollectionClickCallBack : function() {
       ...
       $.ajax({
           type: "post",
           url:'?r=teller/pay-terminus-confirm',
           data:data,
           async:true,
           dataType:'json',
    ...
    _remarkEditClickCallBack: function() {
        ...
        payTerminusDetails.order_id = $(this).data('id');
        $.ajax({
            type: "post",
            url:'?r=teller/order-remark-init',
            data:{id: payTerminusDetails.order_id},
            async:true,
            success: payTerminusDetails._remarkEditSuccess
        });
    },
    ...
    _remarkEditSuccess : function(data) {
        if(data.code == 200) {
            ...
            button.click(payTerminusDetails._remarkSaveClickCallBack);
    ...
    _remarkSaveClickCallBack : function() {
        $.ajax({
            ...
            data:{id: payTerminusDetails.order_id, content: $('#order_remark').val()},
            async:true,
