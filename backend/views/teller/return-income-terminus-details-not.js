var returnIncomeTerminusDetails = {
    order_id:'',
    init : function() {
        this._setEvent();
    },
...
//        $('.return_remark').one('click', this._remarkClickCallBack);
    ...
    _confirmCollectionClickCallBack : function() {
       ...
       $.ajax({
           type: "post",
           url:'?r=teller/return-income-terminus-confirm-not',
           data:data,
           async:true,
           dataType: 'json',
           success:returnIncomeTerminusDetails._confirmSuccess
       });
        ...
        input.blur(function() {
            $.post('?r=teller/return-order-remark', {orderId : orderId, content : input.val()}, function(data){
                if(data.code == 200) {
                    input.remove();
                    td.html(input.val());
                    td.one('click', returnIncomeTerminusDetails._remarkClickCallBack);
                } else {
                    alert(data.msg);
                    input.remove();
                    td.html(content);
    ...
    _remarkEditClickCallBack: function() {
        ...
        returnIncomeTerminusDetails.order_id = $(this).data('id');
        $.ajax({
            type: "post",
            url:'?r=teller/return-order-remark-init',
            data:{id: returnIncomeTerminusDetails.order_id},
            async:true,
            success: returnIncomeTerminusDetails._remarkEditSuccess
        });
    },
    ...
    _remarkEditSuccess : function(data) {
        if(data.code == 200) {
            ...
            button.click(returnIncomeTerminusDetails._remarkSaveClickCallBack);
    ...
    _remarkSaveClickCallBack : function() {
        $.ajax({
            ...
            data:{id: returnIncomeTerminusDetails.order_id, content: $('#order_remark').val()},
            async:true,
...
$(document).ready(function() {
    returnIncomeTerminusDetails.init();
});
