...
//     		in_array($data->goods_state, Yii::$app->params['returnGoodsStateEmployee'],Yii::$app->params['returnGoodsStateFerry'])
    		$arr = array(Yii::$app->params['returnGoodsStateEmployee'],Yii::$app->params['returnGoodsStateFerry']);
    		if(!in_array($data->goods_state, $arr))
    		{
    			return '102';
    		}
    ...
    public function switchUpGoodsState($data, $userId){
    	...
    	switch ($role)
    	{
			...
			case '同城员':
			    if($data->goods_state == Yii::$app->params['returnGoodsStateEmployee'])//状态为10
			    {
    				$r1 = $this->goodsStateCityEdit($data, Yii::$app->params['returnGoodsStateFerry']);//30
    				if($r1)
    				{
    				    return true;
    				}
			...
			case '西部退货组':
			    ...
			    $order = $returnOrder::findOne($data->order_id);//订单model
			    if($data->goods_state == Yii::$app->params['returnGoodsStateEmployee']&&$order->order_state==Yii::$app->params['returnGoodsStateEmployee'])//状态为10
			    {
			        $r1 = $this->goodsStateCityEdit($data, Yii::$app->params['returnGoodsStateFerry']);//30
			        if($r1)
			        {
			    ...
			    if($data->goods_state == Yii::$app->params['returnGoodsStateFerry']&&$order->order_state==Yii::$app->params['returnGoodsStateFerry'])//状态为30
			    {
			        $r1 = $this->goodsStateCityEdit($data, Yii::$app->params['returnGoodsStateDriver']);//50
			        if($r1)
			        {
			            return true;
			        }
			...
			case '瑞胜退货组':
			case '塔湾退货组':
			    if($data->goods_state == Yii::$app->params['returnGoodsStateFerry'])//状态为30
			    {
			        $r1 = $this->goodsStateCityEdit($data, Yii::$app->params['returnGoodsStateDriver']);//50
			        if($r1)
			        {
...
//     	if($data->goods_state == $dataState)//判断状态是否为10
//     	{
