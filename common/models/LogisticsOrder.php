...
class LogisticsOrder extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['order_state', 'state', 'abnormal', 'collection', 'order_type', 'member_id', 'member_cityid','receiving_provinceid', 'receiving_cityid', 'receiving_areaid', 'terminus_id', 'logistics_route_id', 'shipping_type', 'receiving_areaid'], 'integer'],
            [['add_time','logistics_route_id', 'member_cityid', 'member_name', 'member_phone', 'receiving_phone', 'receiving_name'], 'required'],
            [['order_sn','member_name', 'member_phone', 'receiving_name', 'receiving_phone', 'receiving_name_area'], 'string', 'max' => 255],
    ...
    {
    	return Yii::$app->params['city_num'].str_pad($order_id,8,"0",STR_PAD_LEFT);
    }
     ...
     * 朱鹏飞
     */
    public function fillLogisticsInfo(&$model ,$type='member', $username){
        $modelUser = new User();
        ...
        if($type == 'member'){
            ...
            $user_info = $modelUser->getMemberInfo(array('username' =>$username));
            if(empty($user_info)) {
            	return false;
//                 $user_info = $modelUser->userInfo($model, $username);//用户为空，添加新用户
//                 if($user_info == null){
//                 	return false;
//                 }
...
//     	}
    ...
    private function _shippingSale(&$model)
    {
    	...
    	$logistics_route_id = array(2,5,9,12,17,32,14,11,31);//免运费落地点
...
//                 if(!empty($model)&&!($model->state&4)&&$model->collection==1&&empty($model->return_logistics_sn))
     ...
     */
    public function getDriverManagerList($params,$type,$order_arr=null,$add_time = null,$where = null){
        $modelLogisticsRoute = new LogisticsRoute();
        ...
        switch ($type){
            ...
            case 2:
                ...
                $orderList = $this::find()->joinWith('routeInfo')->joinWith('orderTime')
                ->where(['and',
                        ['>=','order_state',Yii::$app->params['orderStateDriver']],
                        'abnormal = 2',
                        $where,
                        $condition,
                        $searchTime
                ])->orderBy('logistics_order.order_id desc');
                return $orderList;
                break;
            case 3:
                $searchTime = $this->getSearchTime($add_time,'add_time');
            ...
            case 4:
                $where = empty($where)?' 1 ':$where;
                $searchTime = $this->getSearchTime($add_time,'ruck_time');
                $orderList = $this::find()->joinWith('routeInfo')->joinWith('orderTime')->joinWith('orderPrintLog')
                ->where([
                        'and',
            ...
            case 5:
                ...
                ->where([
                        ...
                        'abnormal = 2',
                        ['or',
                            ['and','collection = 2','shipping_type=3'],
                            ['and','state <> 1','state <> 2']
                        ],
                        ['or',
                            ['or','collection != 2','shipping_type !=3'],
                            'freight_state != 2'
                        ],
                        $where,
                        $condition,
    ...
    /**
     * 配送票数
     * @param string $driverMemberId  司机id
     * @param  ahthor:齐经纬
     * @param  time：2017-11-07
     */
    public function getMemberOrderNum($driverMemberId){
    	$query = self::find();
    	return $query->where('driver_member_id = :driver_member_id', ['driver_member_id' => $driverMemberId])
    	->count();
    }
    ...
    public function getOrderList($params,$type,$order_arr=null,$add_time = null,$where = null){
        ...
        switch ($type){
            ...
            ->where([
                    'and',
//                     'logistics_order.logistics_route_id = '.$route_id,
                    'order_state = '.Yii::$app->params['orderStateEmployee'],
                    'abnormal = 2',
                    $condition,
                    $searchTime
            ])
            ...
            ->where([
                    ...
                    ['>=','order_state',Yii::$app->params['orderStateDriver']],
                    'abnormal = 2',
                    'logistics_order.logistics_route_id ='.$route_id,$condition,$searchTime,
...
//                 $orderList = $this->stateButtonType($orderList,1);
     ...
     * 靳健
     */
    public function getTerminusList($params,$type,$order_arr = 0,$add_time = null,$driver_id = 0, $print=null){
        $condition = $this->getOrderCondition($params);
        ...
        switch ($type){
            case 1:
                ...
                ->where([
                        ...
                        'order_state = '.Yii::$app->params['orderStateDriver'],
                        'abnormal = 2',
                        'logistics_order.driver_member_id = '.$driver_id,
                        $condition,
                        $searchTime
                ])
                ->orderBy('order_state,logistics_order.order_id desc');
            ...
            case 4:
                ...
                break;
            case 5:
                $orderList = $this::find()->joinWith('routeInfo')->where(['in','order_id',$order_arr])->asArray()->all();
                return $orderList;
                break;
            case 6:
                $searchTime = $this->getSearchTime($add_time,'ruck_time');
                ...
                break;
            case 7:
                $orderList = $this::find()->joinWith('routeInfo')->joinWith('terminusGoodsThree')->joinWith('routeInfo')
                ->where(['and',['in','logistics_order.order_id',$order_arr],$condition])
                ->asArray()->all();
                return $orderList;
                break;
     ...
     * @param unknown $num
     */
    public function getTimeName($num){
       switch ($num){
            case 1: 
                return 'ruck_time';
                break;
...
//         }
     ...
     * 靳健
     */
    public function orderPrint($params,$order_arr,$loading){
        $list = $this->getOrderList($params,8,$order_arr);
        if($loading){
     ...
     * 靳健
     */
    public function getSmallMemo($driver_id){
        $ruck_time = time()-60*100;
        $sql  = "SELECT	*
                FROM
                	`logistics_order` AS l
                RIGHT JOIN (
                	SELECT
                		*
                	FROM
...
//                 'abnormal = 2',
//                 'driver_member_id = '.$driver_id,
//                ['>=','order_time.ruck_time',time()-60*100],
//         ])->asArray()->all();
        return $data;
    }
    ...
    public function isDriverManager($role,$order_id){
        ...
        if($role==Yii::$app->params['roleDriverManagerCityWide']||$role==Yii::$app->params['roleDriverManager']){
            $driver_member_id = $driverConfig->findOne(Yii::$app->user->id)->driver_manager_status;
    ...
    public function tagOrderPrint($order_id){
        $area = new Area();
		$user = new User();
		$paydealer  = new PayDealer();
		$remark = new OrderRemark();
        $data = $this::find()->joinWith('goodsInfo')->joinWith('routeInfo')->where(['logistics_order.order_id'=>$order_id])->asArray()->all();
		 foreach($data as $key => $value){
            $data[$key]['all_amount'] = $paydealer->_getAmount($value)['all_amount'];
        }
    ...
    public function orderToWithdrawal(){
        ...
        try{
            foreach($list as $key => $value){
                ...
                $modelPayDealer = new PayDealer();
                $arr = $modelPayDealer->_getAmount($value);//订单扣出各种价钱的金额
                if(!$withdrawalOrder->addWithdrawalOrder($value['logistics_sn'],1,$value['member_id'],$arr['all_amount'])){
                    return false;
                }
            }
        }catch(Exception $e){
...
// 		->where(['>','add_time',$today])
   ...
   */
    ...
    /*0.0
     * 多表联查，得到申请提现时间、财务付款时间
    */
    public static function getMoreTime($id)
    {
        $query = self::find();
        $result = $query -> select('logistics_order.*,apply_for_withdrawal.add_time,apply_for_withdrawal.pay_time')
            -> innerJoin('withdrawal_order', 'withdrawal_order.order_sn = logistics_order.logistics_sn')
            -> innerJoin('apply_for_withdrawal', 'apply_for_withdrawal.id = withdrawal_order.apply_id')
